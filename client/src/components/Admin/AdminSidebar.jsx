...
export default function AdminSidebar() {
  ...
  return (
    <div className="w-64 bg-slate-900 text-slate-300 flex flex-col h-screen fixed left-0 top-0 shadow-2xl z-50">
      ...
      {/* NÚT VỀ LẠI WEBSITE Ở ĐÂY (Nằm cố định ở dưới cùng) */}
      <div className="p-4 border-t border-slate-800 shrink-0">
        <Link
          to="/"
          className="flex items-center text-slate-400 hover:text-white hover:bg-slate-800 font-bold transition-all duration-200 group px-4 py-3 rounded-xl"
        >
          <ArrowLeft
            size={20}
      ...
      <div className="pb-4 text-xs text-center text-slate-500 shrink-0">
        © 2026 24h Sports
      </div>
    </div>
  );
