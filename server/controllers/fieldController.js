...
exports.getFields = async (req, res) => {
  try {
    // THÊM .populate('category', 'name') ĐỂ LẤY TÊN DANH MỤC
    const fields = await Field.find().populate("category", "name");
    res.status(200).json(fields);
  } catch (error) {
    res
      .status(500)
      .json({ message: "Lỗi khi lấy danh sách sân", error: error.message });
  }
};
...
exports.getFieldById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi khi lấy thông tin sân", error: error.message });
  }
};
...
exports.updateField = async (req, res) => {
  try {
    ...
    if (!updatedField)
      return res
        .status(404)
        .json({ message: "Không tìm thấy sân để cập nhật" });
    res
      .status(200)
      .json({ message: "Cập nhật thành công", field: updatedField });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Lỗi khi cập nhật sân", error: error.message });
  }
};
